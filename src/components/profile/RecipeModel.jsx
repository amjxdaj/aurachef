...
const RecipeModel = ({ recipe, onClose, onFavoriteToggle, isFavorited: initialFavorited }) => {
  ...
  useEffect(() => {
    ...
    const fetchRecipeDetails = async () => {
      try {
        const response = await fetch(
          `https://aurachef-backend.vercel.app/api/recipe/details/${recipe._id}`,
          {
            method: "GET",
            headers: {
    ...
    const fetchFavoriteStatus = async () => {
      try {
        const response = await fetch(
          `https://aurachef-backend.vercel.app/api/favourites/status/${recipe._id}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="md:w-1/2">
            <div className="rounded-lg overflow-hidden shadow-2xl transform hover:scale-[1.02] transition-transform duration-300">
              <img
                src={recipe.image ? `https://aurachef-backend.vercel.app/${recipe.image}` : recipe.image}
